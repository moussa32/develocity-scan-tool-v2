...
const ResultDropdown = ({
  ...
  return (
    <div className={styles.resultRecord} onClick={handleClickOnResult}>
      <div className={styles.resultRecordWrapper}>
        ...
        <div className={styles.details}>
          <div className={styles.headerTitle}>
            ...
            <span className={styles.resultTokenTicker}>
              {symbol}&nbsp;&nbsp;
            </span>
            {isScam && (
              <svg
          ...
          <div className={styles.resultTokenDetails}>
            ...
            {contractScan <= 59 && (
              <div className={`${styles.scanCard} ${styles.scanRed}`}>
                Score:{Math.round(contractScan)}
              </div>
            )}

            {contractScan > 59 && contractScan <= 84 && (
              <div className={`${styles.scanCard} ${styles.scanYellow}`}>
                Score:{Math.round(contractScan)}
              </div>
            )}

            {contractScan > 84 && (
              <div className={`${styles.scanCard} ${styles.scanGreen}`}>
                Score:{Math.round(contractScan)}
              </div>
            )}
      ...
      <button className={`${styles.arrowBtn_ltr}`}>
        <BsArrowRight />
      </button>
    </div>
  );
